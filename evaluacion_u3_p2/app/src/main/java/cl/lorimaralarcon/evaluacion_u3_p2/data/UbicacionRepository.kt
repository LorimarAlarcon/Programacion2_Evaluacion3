...
class UbicacionRepository (
    ...
    suspend fun getUbicacionFromPlayServices(): Location? {
        return withContext(Dispatchers.IO) {
            try {
                val ubicacion = locationService.getCurrentLocation(
    ...
    @SuppressLint("MissingPermission")
    fun getUbicacionFromPlayServices(
        callbackExito: (l: Location) -> Unit,
        callbackError: (e: Exception) -> Unit
    ) {
        val tarea =
            locationService.getCurrentLocation(Priority.PRIORITY_BALANCED_POWER_ACCURACY, null)
